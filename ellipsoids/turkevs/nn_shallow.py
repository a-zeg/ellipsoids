...
def tune_hyperparameters(data_train, labels_train):
    ...
    if "int" in str(type(labels_train[0])) or "str" in str(type(labels_train[0])):
        ...
        nn_shallow = KerasClassifier(model = build_model_classification,
                                     epochs = 2,
                                     verbose = 0,
    ...
    param_grid = {"model__input_layer_width": widths,
                  "model__hidden_layer_width": widths,
    ...
    _, grid_search = model.grid_search(data_train, labels_train, param_grid, nn_shallow)
    ...
    hidden_layer_width_best = grid_search.best_params_["model__hidden_layer_width"]
