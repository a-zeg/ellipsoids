...
# def calculate_3d_point_clouds(point_clouds):  # trying to have a list of numpy arrays, but this doesn't work because of how test / training data is selected in the main script
#     num_samples = len(point_clouds)
#     # num_points = point_clouds[0].shape[0]
#     point_clouds_3d = [None]*num_samples #np.zeros((num_samples, num_points, 3))
#     print('total length:')
#     print(len(point_clouds_3d))
#     for s in range(num_samples):
#         temp_pc_3d_s = []
#         pc = point_clouds[s]               
#         # Original PointNet implementation (https://github.com/charlesq34/pointnet)
...
#             num_points_pc = pc.shape[0]
#             for p in range(num_points_pc):
#                 temp_pc_3d_s.append(np.asarray([pc[p][0], pc[p][1], 0]))
#             point_clouds_3d[s] = np.asarray(temp_pc_3d_s)
#         else:
#             point_clouds_3d[s] = pc
#     # print("point_clouds_3d.shape = ", point_clouds_3d.shape)
#     return point_clouds_3d

def calculate_3d_point_clouds(point_clouds):  # trying to have a numpy array of arrays, all of the same size (truncating the last point)
    num_samples = len(point_clouds)
    num_points = min([len(point_clouds[i]) for i in range(num_samples)])
    print(num_points)
    # num_points = point_clouds[0].shape[0]
    point_clouds_3d = np.zeros((num_samples, num_points, 3))
