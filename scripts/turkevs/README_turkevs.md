...
1. Generate the dataset(s) by running the `generate_turkevs_datasets.py` script;
2. Calculate the ellipsoids (and Rips) barcodes by running the `calculate_ellipsoids_barcodes_turkevs.py` script;
3. Run the experiments by running the `run_classification_experiments.py` script;
4. Calculate average accuracies by running the `calculate_aggregated_results.py` script.
